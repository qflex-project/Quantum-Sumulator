...
void DGM::genMatrix(float complex *matrix, vector<float complex *> &matrices,
...
void PCpuExecution1(float complex *state, PT **pts, int qubits, long n_threads,
  ...
  while (pts[i] != NULL) {
    ...
    while (count < region &&
      ...
      if (  // pts[i]->matrixType() != DIAG_PRI &&					//O qubit de operadores
            // de diagonal principal não importa para região (sempre podem ser
            // acrescentados)
          !((reg_mask >> pts[i]->end) &
        ...
        while (cpu_proj_id != -1) {
          ...
          while (cpu_start < global_end) {
            ...
            long cpu_reg_count =
                ...
                1;  // Número de regiões 			      -	 +1 para a condição de
                    // parada incluir todos
            long cpu_pos_count =
                1 << (cpu_region -
                      1);  // Número de posições na região 	-	 -1 porque são
                           // duas posições por iteração
