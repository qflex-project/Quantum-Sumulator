...
float HadamardNQubits_PAR_CPU(long qubits, long num_of_it, int n_threads,
                              int cpu_region, int cpu_coales) {
  return HadamardNQubits(qubits, num_of_it, t_PAR_CPU, n_threads, cpu_region,
                         cpu_coales, 1, 1, 1, 1, 1);
}

float HadamardNQubits_GPU(long qubits, long num_of_it, int multi_gpu,
                          int gpu_region, int gpu_coales, int tam_block,
                          int rept) {
  return HadamardNQubits(qubits, num_of_it, t_GPU, 1, 1, 1, multi_gpu,
                         gpu_region, gpu_coales, tam_block, rept);
}
