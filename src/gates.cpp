...
bool Gates::addGate(string name, float complex a0, float complex a1,
                    float complex a2, float complex a3) {
  //        cout << "GATES" << endl;
  ...
  cout << "Matrix" << endl;
  cout << crealf(matrix[0]) << " , " << cimagf(matrix[0]) << "\t" <<
  crealf(matrix[1]) << " , " << cimagf(matrix[1]) << endl; cout <<
  crealf(matrix[2]) << " , " << cimagf(matrix[2]) << "\t" << crealf(matrix[3])
  << " , " << cimagf(matrix[3]) << endl << endl;
  */
...
string Controlled2(int qubits, int ctrl1, int ctrl2, int target, string op,
                   int cv) {
...
string ControlledN(int qubits, vector<int> ctrls, int target, string op,
                   int cv) {
...
string concatena(vector<string> vec, int size, bool rev) {
  ...
  if (!rev) {
    s = vec[0];
    for (int i = 1; i < size; i++) s += "," + vec[i];
  } else {
    s = vec[size - 1];
    for (int i = size - 2; i >= 0; i--) s += "," + vec[i];
  }
